...
- `commands/InitCommand.kt`: nothing — the generated template has been `builds`-only since v0.9.19. Verify with `gittally init` in a scratch repo.
...
- Deliberately: point the running instance at a scratch repository whose config still has `branches:` and confirm the error names the file.
